...
# pre-process text
text_process <- function(text, stopwords = TRUE, lower = TRUE) {
...
make_index_list <- function(x, lower = TRUE) {
  data_index <- c()
  
  for (i in 1:length(x)) {
    text <- x[[i]]
    x_seq <- text_to_word_sequence(text, split = " ", lower = lower)
...
predictions_check <- tibble(tweet = test_data, accident = y_test, prediction = predictions[,1]) %>%
  ...
  mutate(predict_accident = replace(predict_accident, prediction > .75, 1)) %>%
...
predict_accident <- function(tweet) {
  ...
  if(prediction > .80) {
    list("This Tweet describes an accident.", tweet, tweet_index, prediction)
  } else {
