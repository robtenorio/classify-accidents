...
truth.df <- read_csv("tweets/tweets_truth_algorithmaccident_geo.csv") %>%
  dplyr::select(created_at, accident_truth, latitude_truth, longitude_truth) %>%
  filter(accident_truth == "Yes") %>%
  mutate(date = ymd_hms(created_at, tz = "Africa/Nairobi")) %>%
  mutate(year = year(date)) %>%
  mutate(month = month(date, label = TRUE)) %>%
  mutate(day = wday(date, label = TRUE)) %>%
  mutate(hour = hour(date)) %>%
  mutate(longitude_truth = as.numeric(longitude_truth)) %>%
  mutate(latitude_truth = as.numeric(latitude_truth))

truth_unique.df <- read_csv("tweets/accidents_unique_algorithm.csv") %>%
  dplyr::select(created_at, accident_truth, latitude_truth, longitude_truth) %>%
  filter(accident_truth == "Yes") %>%
  mutate(date = ymd_hms(created_at, tz = "Africa/Nairobi")) %>%
  mutate(year = year(date)) %>%
