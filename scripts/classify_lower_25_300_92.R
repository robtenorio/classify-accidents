...
# pre-process text
text_process <- function(text, stopwords = FALSE, lower = TRUE) {
...
predictions_check <- tibble(tweet = test_data, accident = y_test, prediction = predictions[,1]) %>%
  ...
  mutate(predict_accident = replace(predict_accident, prediction > .60, 1)) %>%
...
mean(predictions_check$accident[predictions_check$predict_accident == 1])

export_savedmodel(model, "classify_lower_25_300_92")
saveRDS(word_index, "supporting_files/word_index.rds")
...
")
predict_accident("09:17 Bad accident on Outering road(just after Donholm interception towards Taj Mall..upper lane),driver was heading in wrong direction and got up badly badly   @ntsa_kenya   via @JoeNdome")
predict_accident("09:14 An accident at Athi bridge, along Mombasa road this morning and every Kenyan was busy overlapping to nowhere. In a few minutes a gridlock of Jam from both sides of the bridge. quiet annoying ...  https://www.ma3route.com/update/704681   via @OmorisiaChakobo")
predict_accident("06:58 Accident between a mat and a prado hapo barracks. Traffic imeanza mapema leo on langata road via @kagzhun")
predict_accident("06:56 A crash with no injuries at the Muthaiga roundabout, tailback to Gigiri via @MGKwena")
...
predict_accident("06:00 Limuru road inbound Muthaiga round about there's an accident drive carefully. Car being towed via @robertj800")
predict_accident("05:52 Accident at safaripark superhighway but police are on the scene   via @78mashawilliam")
...
predict_accident("This is sad. Pple r always driving on the wrong side especially from fedha to doni.")
predict_accident("08:43 @muthoka_tom Juja road flowing quite well. No incidents. The road at mlango kubwa is a shamba. Very bad. Otherwise nothing major to write home about via @FKIMATU")
predict_accident("08:40 #KBV866F @ntsa_kenya I need assistance with this matatu plying thika rd, Purple 33 seater minibus. Hit and broke the side mirror of my car and took off nearly crashing me on the road as he tried to escape.   via @MateMurithi")
predict_accident("08:15 @KeNHAKenya Big hole emerging at kariguini blackspot downhill Kenol-Sabasaba busy road! Will it be fixed when accident happen? via @johngitau17")
...
predict_accident("07:21 Mombasa rd at stone athi river is a no go zone courtesy of   via @MutukuMulwa2")
