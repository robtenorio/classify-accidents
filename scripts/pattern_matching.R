...
### tweets
tweets <- read_csv("tweets/truthdata_allrounds.csv") %>% select(id, tweet, accident_truth)
...
####### test
df.tweets_test <- df.tweets %>% 
  mutate(accident = factor(accident_exact_match, labels = c("No", "Yes"))) %>%
  mutate(correct = case_when(accident_truth == accident ~ 1)) %>%
  mutate(correct = replace(correct, is.na(correct), 0))

mean(df.tweets_test$correct)

table(df.tweets_test$accident[df.tweets_test$correct == 0])
View(df.tweets_test[df.tweets_test$correct == 0,])
