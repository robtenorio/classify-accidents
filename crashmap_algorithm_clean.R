...
crashmap_algorithm <- function(tweets,
  ...
  if(remove.street.names){
    street.names <- read.csv("~/Dropbox/World Bank/CrashMap-Nairobi/Algorithm/code/Main Algorithm/Supporting Files/google_street_names.csv")
  ...
  if(levenstein.word){
    ...
    if(nrow(df.tweets_nogeo) > 0){
      ...
      tweet_df_leven_word <- lapply(df.tweets_nogeo$tweet_clean, 
                                    levenstein_word, 
                                    landmark_word_df, 
                                    landmarks_simple, 
                                    equal.distant.projection,
                                    ...
                                    levenstein.word.min.words.fuzzy,
                                    levenstein.word.max.word.distance,
                                    multiple_landmark_distance_threshold_km) %>% bind_rows
    
      # Merge results back to dataframe with all tweets
      names(tweet_df_leven_word) <- c("lat_leven_word","lon_leven_word","word_in_tweets_matched_leven_word","landmarks_leven_word","landmarks_close_leven_word","multiple_landmarks_max_dist","multiple_landmarks_mean_dist","multiple_landmarks_min_dist","multiple_landmarks_number","multiple_landmarks_number_pairs","multiple_landmarks_num_pairs_below_thresh")
      
      tweet_df_leven_word$id <- df.tweets_nogeo$id
  ...
  if(nrow(landmark_match) > 0){
    ...
    if(nrow(landmark_match) >= 2){
      if(1 %in% (landmark_match$word_number - tweet_prepositions_loc$word_number)){
        landmark_match <- landmark_match[(landmark_match$word_number - tweet_prepositions_loc$word_number) == 1,]
      }
    }
...
# DELETE - - - - - - - - - - -
if(F){
  tweet <- tweets_clean_nogeo$tweet_clean[1]
  tweet_clean <- tweet
